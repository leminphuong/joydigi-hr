...
from joydigi_audit.methods import remove_duplicate_history
...
class JoydigiAuditInfo(models.Model):
    """
    JoydigiAuditInfo model to store additional info
    """
...
class JoydigiAuditLog(HistoricalRecords):
    """
    Model to store additional information for historical records.
    """

    # def __init__(self, *args, bases=None, **kwargs):
    #     super(JoydigiAuditLog, self).__init__(*args, **kwargs)
    #     self.is_joydigi_audit_log = True
...
def pre_create_joydigi_audit_log(sender, instance, *args, **kwargs):
    """
...
def post_create_joydigi_audit_log(sender, instance, *_args, **kwargs):
    """
    ...
    try:
        ...
        if isinstance(history_instance, JoydigiAuditLog):
...
class HistoryTrackingFields(JoydigiModel):
    """
...
class AccountBlockUnblock(JoydigiModel):
    is_enabled = models.BooleanField(default=False, null=True, blank=True)
    objects = models.Manager()


class AuditModelConfig(JoydigiModel):
    """
