"""
Models for the joydigi_theme app
"""

from django.core.exceptions import ValidationError
from django.core.validators import RegexValidator

# Create your joydigi_theme models here.
...
class JoydigiColorTheme(JoydigiModel):
    """
    Model to store predefined color themes for Joydigi
    """
    ...
    class Meta:
        """
        Meta option for JoydigiColorTheme model
        """
    ...
    def save(self, *args, **kwargs):
        ...
        with transaction.atomic():
            if self.is_default:
                query = JoydigiColorTheme.objects.filter(is_default=True)
                if self.pk:
                    query = query.exclude(pk=self.pk)
                query.update(is_default=False)
            super().save(*args, **kwargs)
            if self.is_default and self.pk:
                JoydigiColorTheme.objects.filter(is_default=True).exclude(
                    pk=self.pk
...
class CompanyTheme(JoydigiModel):
    """
    ...
    def get_default_theme(cls):
        ...
        Returns the theme object or None
        """
        return JoydigiColorTheme.get_default_theme()
    ...
    def get_theme_for_company(cls, company):
        ...
        # return JoydigiColorTheme.objects.filter(
        #     name="Coral Red Theme (Default)"
        # ).first()
        return JoydigiColorTheme.objects.filter(is_default=True).first()
