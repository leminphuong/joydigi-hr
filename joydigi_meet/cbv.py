...
from joydigi_meet.filters import GoogleMeetingFilter
from joydigi_meet.form import GoogleCloudCredentialForm, GoogleMeetingForm
from joydigi_meet.models import GoogleCloudCredential, GoogleCredential, GoogleMeeting
...
class GMeetCredentialSectionView(views.JoydigiSectionView):
    """
    View for displaying the Google Meet credentials section.

    Inherits from `JoydigiSectionView` to manage the section for Google Cloud credentials in the user interface.
...
@method_decorator(login_required, name="dispatch")
@method_decorator(check_integration_enabled(app_name="joydigi_meet"), name="dispatch")
@method_decorator(
    permission_required("joydigi_meet.view_googlecloudcredential"), name="dispatch"
)
class GmeetCredentialNavView(views.JoydigiNavView):
    """
    View for the Google Meet credentials nav view.

    Inherits from `JoydigiNavView` to manage nav view for Google Cloud credentials. Handles dynamic attributes
...
class GmeetCredentialListView(views.JoydigiListView):
    """
    View for listing Google Meet credentials.

    Inherits from `JoydigiListView` to manage the listing of Google Cloud credentials with actions like edit and delete.
...
@method_decorator(check_integration_enabled(app_name="joydigi_meet"), name="dispatch")
@method_decorator(
    permission_required("joydigi_meet.add_googlecloudcredential"), name="dispatch"
)
class GoogleCredentialsFormView(views.JoydigiFormView):
    """
    View for creating or updating Google Cloud credentials.

    Inherits from `JoydigiFormView` to manage the form for creating or updating Google Cloud credentials.
...
@method_decorator(login_required, name="dispatch")
@method_decorator(
    permission_required("joydigi_meet.add_googlemeeting"), name="dispatch"
)
@method_decorator(check_integration_enabled(app_name="joydigi_meet"), name="dispatch")
class GmeetFormView(views.JoydigiFormView):
    """
    View for creating or updating Google Meet meetings.

    Inherits from `JoydigiFormView` to manage the form for creating or updating Google Meet meetings.
    Handles form validation, success, and failure responses. Displays success or error messages accordingly.
...
@method_decorator(check_integration_enabled(app_name="joydigi_meet"), name="dispatch")
@method_decorator(
    permission_required("joydigi_meet.view_googlemeeting"), name="dispatch"
)
class GmeetSectionView(views.JoydigiSectionView):
    """
    View for displaying the Google Meet section.

    Inherits from `JoydigiSectionView` to manage the section for Google Meet in the user interface.
...
class GmeetNavView(views.JoydigiNavView):
    """
    Nav view for Google Meet.

    Inherits from `JoydigiNavView` to manage nav bar for Google Meet view. Handles dynamic attributes for creating
...
class GmeetListView(views.JoydigiListView):
    """
    View for listing Google Meetings.

    Inherits from `JoydigiListView` to manage the listing of Google meetings with actions like edit, delete, and view meeting link.
...
class GmeetDetailedView(views.JoydigiDetailedView):
    """
    Detailed view for Google Meetings.

    Inherits from `JoydigiDetailedView` to manage the detailed view of a specific Google Meeting.
