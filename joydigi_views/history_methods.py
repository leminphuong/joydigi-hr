...
from joydigi.decorators import apply_decorators
...
class Bot:
    ...
    def get_avatar(self):
        ...
        Get avatar
        """
        return "https://ui-avatars.com/api/?name=Joydigi+Bot&background=random"
...
def get_diff(instance, history_related_name):
    ...
    for pair in pairs:
        ...
        if not diffs:
            continue
        updated_by = (
            JoydigiUser.objects.get(id=pair[0].history_user.id).employee_get
    ...
    # simple-history) have no rows — still surface creation from JoydigiModel.
    ...
    if instance._meta.model_name == "employeeworkinformation":
        from joydigi_audit.models import HistoryTrackingFields
...
def history_tracking(request, obj_id, **kwargs):
    ...
    def _history_tracking(request, obj_id, model):
        ...
        histories = instance.joydigi_history.all()
        ...
        return render(
            request,
            "joydigi_audit/history_tracking.html",
            context,
        )
