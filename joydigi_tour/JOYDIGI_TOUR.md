# Joydigi Tour & Getting Started Checklist

Functional and technical reference for Joydigi’s onboarding UX:

1. **Product Tours** (`joydigi_tour`) — guided, driver.js walkthroughs on any page
2. **Setup Checklist** — “Get started with Joydigi HR” banner on the dashboard (8 setup steps)
...
## Part 1 — Product Tours (`joydigi_tour`)
...
1. Authenticated page load injects `window.JOYDIGI_TOUR` and `tourController.js`
...
│    window.JOYDIGI_TOUR = { page, path, activeUrl, … }       │
...
└──────────────────────────┬──────────────────────────────────┘
                           │
┌──────────────────────────▼──────────────────────────────────┐
│  Models (JoydigiModel + JoydigiCompanyManager)              │
...
- Installed app: `joydigi_tour` (`joydigi/settings/base.py`)
- URLs: `path("", include("joydigi_tour.urls"))` in `joydigi/urls.py`
- Context processor: `joydigi_tour.context_processors.pending_tours_flag`
  - Exposes `tour_launcher_enabled`, `tour_has_pending` for the header Help button
- Settings menu: `joydigi_tour/sidebar.py` → “Product Tours / Manage Tours”
- Scripts: `joydigi_theme/.../footer_scripts.html` (authenticated only)
...
| `tour-nav` / `tour-list` | JoydigiNavView / JoydigiListView | `view_tour` |
| `tour-create-form` / `tour-update-form` | JoydigiFormView | `add_tour` / change via update |
| `tour-steps` / `tour-step-form` / `tour-step-delete` | HTMX panel | `view_tour` / `change_tour` |

Patterns: JoydigiListView row actions, HTMX modals, generic delete.

#### Frontend (`tourController.js`)

- Depends on global **driver.js** (`window.driver.js.driver`)
- Reads `window.JOYDIGI_TOUR`
- Builds driver steps from API payload
- On destroy: posts `completed` if last step reached, else `skipped`
- Exposes launcher UI (`#joydigiTourLauncher`) and `window.joydigiTour` for Start/Replay
- Header button `#tourLauncherBtn` toggles the panel

#### Permissions

Django model perms on `joydigi_tour.Tour`:
...
## Part 2 — Setup Checklist (“Get started with Joydigi HR”)

Dashboard onboarding banner that walks admins through foundational HR configuration. Not part of `joydigi_tour`, but the primary “first run” tour of the product.

### Functional overview

**Copy & UX**

- Title: **Get started with Joydigi HR**
...
`_exists_for_company` mirrors `JoydigiCompanyManager`: record counts if assigned to company **or** company FK/M2M is null (shared lookup data).
...
|---|---|---|
| App | `joydigi_tour` | `base` (+ dashboard) |
...
joydigi_theme/.../footer_scripts.html   JOYDIGI_TOUR bootstrap
joydigi_theme/.../header.html           Take a tour button
```
...
6. **Permissions** — grant `joydigi_tour.view_tour` (and change/add) only to admins who should author tours.
`)
