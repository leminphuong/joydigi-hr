...
from joydigi.filters import FilterSet
...
from joydigi.signals import post_generic_import, pre_generic_import
from joydigi_views import models
from joydigi_views.cbv_methods import (  # update_initial_cache,
    assign_related,
    export_xlsx,
...
from joydigi_views.forms import DynamicBulkUpdateForm, ToggleColumnForm
from joydigi_views.related_link_registry import RELATED_VIEW_PARAM, register_detail_view
...
def model_has_audit_tracking(model) -> bool:
    ...
    ``joydigi_history`` field every ``JoydigiModel`` subclass gets.
    """
    return (
...
def register_dynamic_url(route: str, view, name: str = None) -> None:
    ...
    from joydigi.urls import path, urlpatterns
...
class JoydigiListView(ListView):
    """
    JoydigiListView
    """
    ...
    template_name: str = "generic/joydigi_list_table.html"
    ...
    def get_context_data(self, **kwargs: Any):
        ...
        if (
            ...
            and issubclass(self.model, JoydigiModel)
        ...
        if request and self._saved_filters.get("field"):
            ...
            try:
                ...
                self.template_name = "generic/joydigi_list_table.html"
        ...
        # CACHE.get(self.request.session.session_key + "cbv")[JoydigiListView] = context
        self.export_path = (
...
class JoydigiSectionView(TemplateView):
    """
    Joydigi Template View
    """
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        request = getattr(_thread_locals, "request", None)
        self.request = request
        # update_initial_cache(request, CACHE, JoydigiListView)
    ...
    template_name = "generic/joydigi_section.html"
...
@method_decorator(hx_request_required, name="dispatch")
class JoydigiDetailedView(DetailView):
    """
    HorillDetailedView
    """

    title = _("Detailed View")
    template_name = "generic/joydigi_detailed_view.html"
    header: dict = {
        "title": "Joydigi",
        "subtitle": "Joydigi Detailed View",
        "avatar": "",
    }
    body: list = []
    ...
    # target for `model` (see joydigi_views/related_link_registry.py), instead
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        request = getattr(_thread_locals, "request", None)
        self.request = request
        # update_initial_cache(request, CACHE, JoydigiDetailedView)
...
@method_decorator(hx_request_required, name="dispatch")
class JoydigiTabView(TemplateView):
    """
    JoydigiTabView
    """

    view_id: str = ""
    template_name = "generic/joydigi_tabs.html"
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        # update_initial_cache(request, CACHE, JoydigiTabView)
    ...
    def get_context_data(self, **kwargs):
        ...
        # CACHE.get(self.request.session.session_key + "cbv")[JoydigiTabView] = context

        return context


@method_decorator(hx_request_required, name="dispatch")
class JoydigiCardView(ListView):
    """
    JoydigiCardView
    """
    ...
    template_name = "generic/joydigi_card.html"
    context_object_name = "queryset"
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        request = getattr(_thread_locals, "request", None)
        self.request = request
        # update_initial_cache(request, CACHE, JoydigiCardView)
    ...
    def get_context_data(self, **kwargs):
        ...
        # CACHE.get(self.request.session.session_key + "cbv")[JoydigiCardView] = context
        referrer = self.request.GET.get("referrer", "")
        if referrer:
            # Remove the protocol and domain part
            referrer = "/" + "/".join(referrer.split("/")[3:])
...
@method_decorator(hx_request_required, name="dispatch")
class JoydigiFormView(FormView):
    """
    JoydigiFormView
    """
    ...
    template_name = "generic/joydigi_form.html"
    ids_key: str = "instance_ids"
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        # update_initial_cache(request, CACHE, JoydigiFormView)
        ...
        if not isinstance(existing_form, forms.BaseForm):
            ...
            if (
                ...
                if isinstance(company_field, forms.ModelMultipleChoiceField):
                    ...
                    form.initial["company_id"] = selected_company
            # CACHE.get(self.request.session.session_key + "cbv")[JoydigiFormView] = form
            self.form = form
        return self.form
...
@method_decorator(hx_request_required, name="dispatch")
class JoydigiNavView(TemplateView):
    """
    JoydigiNavView
    ...
    template_name = "generic/joydigi_nav.html"
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        request = getattr(_thread_locals, "request", None)
        self.request = request
        # update_initial_cache(request, CACHE, JoydigiNavView)
    ...
    def get_context_data(self, **kwargs):
        ...
        # CACHE.get(self.request.session.session_key + "cbv")[JoydigiNavView] = context
        return context
...
@method_decorator(hx_request_required, name="dispatch")
class JoydigiProfileView(DetailView):
    """
    GenericJoydigiProfileView
    """

    template_name = "generic/joydigi_profile_view.html"
    view_id: str = None
    filter_class: FilterSet = None
    push_url: str = None
    key_name: str = "pk"
    ...
    # joydigi_views/urls.py — see dispatch_profile_tab() below for why this
    ...
    def _register_tabs(cls) -> None:
        ...
        for tab in cls.tabs:
            if not tab.get("url"):
                ...
                JoydigiProfileView._tab_view_registry[key] = tab["view"]
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        self.request = request
        self.ordered_ids_key = f"ordered_ids_{self.model.__name__.lower()}"
        # update_initial_cache(request, CACHE, JoydigiProfileView)
...
def dispatch_profile_tab(request, tab_key: str, pk: int, *args, **kwargs):
    ...
    joydigi_views/urls.py) that every JoydigiProfileView tab (candidate
    profile, employee profile, etc.) resolves through.

    JoydigiProfileView used to append a brand new URL pattern to the live
    ...
    in JoydigiProfileView._tab_view_registry at request time removes both
    problems: the registry is fully populated before Django serves its first
    request (see JoydigiProfileView._register_tabs), the same as it would be
    in every worker process.
    """
    view_func = JoydigiProfileView._tab_view_registry.get(tab_key)
