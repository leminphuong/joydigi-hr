...
2. Settings admin CRUD (permission-gated) to author tours and their ordered
   steps, following the JoydigiSectionView + JoydigiNavView + JoydigiListView +
   JoydigiFormView pattern used elsewhere in Settings.
"""
...
from joydigi_tour.filters import TourFilter
from joydigi_tour.forms import TourForm, TourStepForm
from joydigi_tour.models import Tour, TourProgress, TourStep
...
@auth_login_required
@permission_required("joydigi_tour.view_tour")
def tour_settings_page(request):
    """Render the Tour Management page inside the Settings shell."""
    return render(request, "joydigi_tour/tour_settings.html")


@method_decorator(login_required, name="dispatch")
@method_decorator(permission_required("joydigi_tour.view_tour"), name="dispatch")
class TourNav(JoydigiNavView):
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        if self.request.user.has_perm("joydigi_tour.add_tour"):
            self.create_attrs = f"""
...
@method_decorator(login_required, name="dispatch")
@method_decorator(permission_required("joydigi_tour.view_tour"), name="dispatch")
class TourList(JoydigiListView):
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        if self.request.user.has_perm("joydigi_tour.change_tour"):
            self.actions.insert(
                0,
                {
                    "action": _("Edit"),
        ...
        if self.request.user.has_perm("joydigi_tour.delete_tour"):
            self.actions.append(
                {
                    "action": _("Delete"),
                    "icon": "trash-outline",
                    "attrs": """
                        ...
                        hx-get="{get_delete_url}?model=joydigi_tour.tour&pk={pk}"
                        data-toggle="oh-modal-toggle"
...
@method_decorator(login_required, name="dispatch")
@method_decorator(permission_required("joydigi_tour.add_tour"), name="dispatch")
class TourFormView(JoydigiFormView):
...
def _render_steps_panel(request, tour):
    return render(
        request,
        "joydigi_tour/steps_panel.html",
        {
            "tour": tour,
            "steps": tour.steps.all(),
            "can_edit": request.user.has_perm("joydigi_tour.change_tour"),
        },
    )
...
@auth_login_required
@permission_required("joydigi_tour.change_tour")
