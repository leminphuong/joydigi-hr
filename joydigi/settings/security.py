...
Applied from base settings when DEBUG is False or JOYDIGI_ENV=production.
...
def is_production_mode(debug: bool, joydigi_env: str) -> bool:
    return (not debug) or (joydigi_env or "").strip().lower() == "production"
...
def validate_production_secrets(secret_key, allowed_hosts, db_init_password):
    ...
    if errors:
        raise ImproperlyConfigured(
            "Joydigi production security check failed:\n- " + "\n- ".join(errors)
        )
