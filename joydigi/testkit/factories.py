"""Lightweight model factories for Joydigi TestCase suites."""
...
def make_user(
    ...
    *,
    ...
    email = email or f"{username}@test.joydigi"
    if is_superuser:
        return JoydigiUser.objects.create_superuser(
            username=username, email=email, password=password
        )
    return JoydigiUser.objects.create_user(
        username=username, email=email, password=password
    )
...
def make_employee(
    *,
    ...
    Employee.save() auto-creates a JoydigiUser when none is set; pass
